...
class ShapeDetectionNode(Node):
    def __init__(self):
        ...
        # Create a publisher for the shape data custom message
        # self.shape_pub = self.create_publisher(ShapeData, 'shape_detection/shape_info', 10)
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            # Xử lý hình ảnh
            cv_image = cv2.resize(frame_rgb, (0, 0), fx=0.5, fy=0.5)
            processed_image = self.anisotropic_diffusion(cv_image)
            
            # Phát hiện hình tròn
            blur_frame, mask_blue, mask_red, mask_yellow, mask_black, blue_on_black, red_on_black, yellow_on_black, contour_black = self.process_frame(processed_image)
            
            # Vẽ kết quả phát hiện lên ảnh
            output, circle_info = self.draw_circles_on_frame(blur_frame, blue_on_black, red_on_black, yellow_on_black, contour_black)
    ...
    def detect_blue_regions(self, frame, area_threshold=None):
                # Đảm bảo frame là BGR trước khi chuyển đổi
        if frame.shape[2] == 3:  # RGB
            frame_bgr = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
        else:
            frame_bgr = frame
        
        # Sau đó chuyển sang HSV
    ...
    def detect_circles_on_black_background(self, circles_blue, circles_red, circles_yellow, frame, area_threshold=None):
        ...
        upper_black = np.array([180, 100, 60])  # Phạm vi rộng hơn cho màu đen
        ...
        def filter_circles(circles):
            valid = []
            if circles is not None and black_mask is not None:
                for (x, y, r) in circles:
                    # Kiểm tra xem tâm của vòng tròn có nằm trong vùng đen không
                    if black_mask[int(y), int(x)] > 0:
                        valid.append([int(x), int(y), int(r)])
            return valid
        
        # Lọc các vòng tròn theo từng màu
        valid_blue = filter_circles(circles_blue)
        valid_red = filter_circles(circles_red)
        valid_yellow = filter_circles(circles_yellow)
    ...
    def process_frame(self, frame_o):
        ...
        blue_on_black, red_on_black, yellow_on_black, contour_black, mask_black = self.detect_circles_on_black_background(
        ...
        return frame, mask_blue, mask_red, mask_yellow, mask_black, blue_on_black, red_on_black, yellow_on_black, contour_black
    ...
    def get_color_name(self, r, g, b):
        ...
        hsv = cv2.cvtColor(color_rgb, cv2.COLOR_BGR2HSV)[0][0]
        h, s, v = hsv
    ...
    def get_dominant_color_name(self, roi):
        """ Return the most dominant color name in the ROI area, ignoring black pixels """
        if roi.size == 0:  # Check if ROI is empty
            return "Unknown", (0, 0, 0)
            
        # Tạo mặt nạ cho các pixel không đen (có giá trị >10 trong tất cả các kênh)
        mask = np.all(roi > 10, axis=2)
        
        # Nếu không có pixel nào thỏa mãn điều kiện
        if np.sum(mask) == 0:
            return "Unknown", (0, 0, 0)
        
        # Chỉ lấy các pixel không đen để tính màu trung bình
        non_black_pixels = roi[mask]
        avg_color = np.mean(non_black_pixels, axis=0)
        b, g, r = int(avg_color[0]), int(avg_color[1]), int(avg_color[2])
        
        return self.get_color_name(r, g, b), (b, g, r)
    ...
    def draw_circles_on_frame(self, frame, blue_circles=None, red_circles=None, yellow_circles=None, contour_black=None):
        ...
        if blue_circles is not None:
            ...
            for (x, y, r) in blue_circles:
                cv2.circle(output, (x, y), r, (255, 0, 0), 2)  # BGR format - Blue
                # Hiển thị tâm đường tròn rõ ràng hơn
                cv2.circle(output, (x, y), 5, (0, 255, 255), -1)  # Tâm màu vàng lớn hơn
                cv2.drawMarker(output, (x, y), (0, 255, 255), cv2.MARKER_CROSS, 10, 2)  # Dấu chéo tại tâm
                # Hiển thị tọa độ tâm
                cv2.putText(output, f"({x},{y})", (x + 10, y), 
        ...
        if red_circles is not None:
            ...
            for (x, y, r) in red_circles:
                cv2.circle(output, (x, y), r, (0, 0, 255), 2)  # BGR format - Red
                # Hiển thị tâm đường tròn rõ ràng hơn
                cv2.circle(output, (x, y), 5, (0, 255, 255), -1)  # Tâm màu vàng lớn hơn
                cv2.drawMarker(output, (x, y), (0, 255, 255), cv2.MARKER_CROSS, 10, 2)  # Dấu chéo tại tâm
                # Hiển thị tọa độ tâm
        ...
        if yellow_circles is not None:
            ...
            for (x, y, r) in yellow_circles:
                cv2.circle(output, (x, y), r, (0, 255, 255), 2)  # BGR format - Yellow
                # Hiển thị tâm đường tròn rõ ràng hơn
                cv2.circle(output, (x, y), 5, (255, 0, 0), -1)  # Tâm màu xanh lớn hơn
                cv2.drawMarker(output, (x, y), (255, 0, 0), cv2.MARKER_CROSS, 10, 2)  # Dấu chéo tại tâm
                # Hiển thị tọa độ tâm
                cv2.putText(output, f"({x},{y})", (x + 10, y), 
                            cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 255), 1)
        ...
        # Trong hàm draw_circles_on_frame
        if largest_circle is not None:
            x, y, r = largest_circle
            # ...
            
            # Cải thiện việc trích xuất ROI
            mask = np.zeros((output.shape[0], output.shape[1]), dtype=np.uint8)
            cv2.circle(mask, (x, y), int(r * 0.8), 255, -1)  # Giảm kích thước vùng trích xuất để tránh biên
            
            # Áp dụng mặt nạ trên frame gốc (ít bị nhiễu hơn)
            frame_copy = frame.copy()  # Sử dụng frame gốc thay vì output đã có vẽ
            ...
            if x1 < x2 and y1 < y2:
                ...
                color_name, dominant_bgr = self.get_dominant_color_name(roi)
                        # Kiểm tra màu dựa trên các tập hợp vòng tròn gốc
                        
                color_name = "Unknown"
                if blue_circles is not None and any(np.all(np.array([x, y, r]) == circle) for circle in blue_circles):
                    color_name = "Blue"
                elif red_circles is not None and any(np.all(np.array([x, y, r]) == circle) for circle in red_circles):
                    color_name = "Red"
                elif yellow_circles is not None and any(np.all(np.array([x, y, r]) == circle) for circle in yellow_circles):
                    color_name = "Yellow"
            
                # Thêm bước kiểm tra xác minh màu
                if color_name == "Unknown":
                    # Kiểm tra xem hình tròn này thuộc tập nào
                    if blue_circles is not None and any(np.linalg.norm(np.array([x,y]) - np.array([cx,cy])) < 10 
                                                    for (cx,cy,_) in blue_circles):
                        color_name = "Blue"
                        dominant_bgr = (255, 0, 0)
                    elif red_circles is not None and any(np.linalg.norm(np.array([x,y]) - np.array([cx,cy])) < 10 
                                                    for (cx,cy,_) in red_circles):
                        color_name = "Red"
                        dominant_bgr = (0, 0, 255)
                    elif yellow_circles is not None and any(np.linalg.norm(np.array([x,y]) - np.array([cx,cy])) < 10 
                                                        for (cx,cy,_) in yellow_circles):
                        color_name = "Yellow"
                        dominant_bgr = (0, 255, 255)
                ...
                shape_info.shape_name = color_name
